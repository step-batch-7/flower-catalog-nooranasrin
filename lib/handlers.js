...
const loadPreviousFeedbacks = function() {
  ...
  if (isFileNotAvailable(dataStoragePath)) {
    return [];
  }
...
const saveComments = function(request, response) {
  ...
  redirect(response, 'guestBook.html');
};

const methodNotAllowed = function(request, response) {
  const statusCode = STATUS_CODE.METHOD_NOT_ALLOWED;
  sendResponse(response, 'text/plain', statusCode, 'Method Not Allowed');
};
